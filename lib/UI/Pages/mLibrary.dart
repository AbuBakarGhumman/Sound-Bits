...
import '../Components/albums_item.dart';
import '../Components/artist_item.dart';
import '../Components/artists_item.dart';
...
class _MLibraryPageState extends State<MLibraryPage> {
  ...
  late Future<Map<String, List<Song>>> _artistsFuture;
  ...
  void didChangeDependencies() {
    ...
    if (!_futuresInitialized) {
      ...
      _artistsFuture = _musicService.fetchArtists();
      ...
      case 'Albums':
        return FutureBuilder<Map<String, List<Song>>>(
          ...
          builder: (context, snapshot) {
            ...
            return ListView.builder(
              key: const ValueKey('artists_list'),
              padding: const EdgeInsets.only(top: 12.0, bottom: 90.0),
              itemCount: albumNames.length,
              itemBuilder: (context, itemIndex) {
                final albumName = albumNames[itemIndex];
                final songsInAlbum = albums[albumName]!;
                final firstSong = songsInAlbum.first;

                return AlbumsItem(
                  folderName: albumNames[itemIndex] == "<unknown>" ? "Unknown" : albumNames[itemIndex],
                  trackCount: firstSong.artist == "<unknown>" ? "Unknown Artist" : firstSong.artist,
                  isDark: isDark,
                  onTap: () {
      ...
      case 'Artists':
        return FutureBuilder<Map<String, List<Song>>>(
          ...
          builder: (context, snapshot) {
            ...
            final folderPaths = artists.keys.toList();
            ...
            return ListView.builder(
              ...
              itemBuilder: (context, itemIndex) {
                ...
                final songsInartist = artists[folderPath]!;

                return ArtistsItem(
                  folderName: p.basename(folderPath) == "<unknown>" ? "Unknown Artist" : p.basename(folderPath),
                  trackCount: songsInartist.length,
                  isDark: isDark,
                  onTap: () {
                    if (songsInartist.isNotEmpty) {
                      widget.onOpenTrackList(
                          p.basename(folderPath), songsInartist);
                    }
                  },
                );
              },
        ...
        /*return FutureBuilder<Map<String, List<Song>>>(
          ...
          builder: (context, snapshot) {
            ...
            return GridView.builder(
              key: const ValueKey('artists_grid'),
              padding: const EdgeInsets.all(12.0),
              gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                crossAxisCount: 2,
                crossAxisSpacing: 12,
              ...
              itemBuilder: (context, index) {
                ...
                final songsByArtist = artists[artistName]!;
                final firstSong = songsByArtist.first;

                return ArtistItem(
                  albumTitle: artistName, // Using AlbumItem for artist
                  coverUrl: null,
                  isDark: isDark,
                  onTap: () {
                    widget.onOpenTrackList(artistName, songsByArtist);
                  },
                );
              },
            );
            ...
            Padding(
              ...
              child: Row(
                ...
                children: [
                  ...
                  // Conditionally hide the icon for Folders, Albums, or Artists
                  (tabs[selectedIndex] == 'Folders' ||
                      tabs[selectedIndex] == 'Albums' ||
                      tabs[selectedIndex] == 'Artists')
                      ? Container()  // Empty container when selected tab is Folders, Albums, or Artists
                      : Container(
